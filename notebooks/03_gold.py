...
# gold paths
gold_dim_patient = "abfss://<<Container_name>>@<<Storage_accountname>>.dfs.core.windows.net/<<path>>"
gold_dim_department = "abfss://<<Container_name>>@<<Storage_accountname>>.dfs.core.windows.net/<<path>>"
gold_fact = "abfss://<<Container_name>>@<<Storage_accountname>>.dfs.core.windows.net/<<path>>"
...
# Rank by latest admission_time and Keep only latest admission
silver_df = (
    silver_df
...
fact_enriched = fact_enriched.withColumn("length_of_stay_hours",
                                         F.round((unix_timestamp(col("discharge_time")) - F.unix_timestamp(col("admission_time"))) / 3600.0,2)) \
